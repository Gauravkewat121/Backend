...
Include movie name and total tickets sold.


select m.name,count(*)as total_tickets
from (
	select mt.movie_id movie_id
	from MovieTheaters mt join Bookings b 
	on mt.MT_id = b.MT_id
	where b.status = 'booked' And b.isDeleted = 0
	group by mt.MT_id,mt.movie_id
) as t
join Movies m
...
lowest.
 ...
 select mt.theater_id , mt.screen_id, sum(b.total_amount) as totalAmount 
 from MovieTheaters mt
 join Bookings b
 on b.MT_id = mt.MT_id 
 where b.status = 'booked' and mt.isDeleted = 0
 group by mt.theater_id,mt.screen_id
